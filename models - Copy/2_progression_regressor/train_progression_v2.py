...
class ProgressionTrainer:
    ...
    def train_lightgbm_regression(self, X_train, y_train, X_test, y_test):
...
def main():
    ...
    # log_experiment('progression-regression-v2')
    
    # Train models
    print("\n" + "=" * 80)
    print("TRAINING MODELS WITH ENRICHED FEATURES")
    print("=" * 80)
    ...
    # 1. Linear Regression
    results = trainer.train_linear_regression(X_train_scaled, y_train, X_test_scaled, y_test)
    trainer.print_results('LinearRegression-v2', results)
    results_dict['LinearRegression-v2'] = results
    
    if results['test_r2'] > best_r2:
        best_r2 = results['test_r2']
        best_model_name = 'LinearRegression-v2'
    
    # 2. Ridge Regression (alpha=0.1)
    results = trainer.train_ridge_regression(X_train_scaled, y_train, X_test_scaled, y_test, alpha=0.1)
    trainer.print_results('RidgeRegression-v2-alpha01', results)
    results_dict['RidgeRegression-v2-alpha01'] = results
    
    if results['test_r2'] > best_r2:
        best_r2 = results['test_r2']
        best_model_name = 'RidgeRegression-v2-alpha01'
    
    # 3. Ridge Regression (alpha=1.0)
    results = trainer.train_ridge_regression(X_train_scaled, y_train, X_test_scaled, y_test, alpha=1.0)
    trainer.print_results('RidgeRegression-v2-alpha10', results)
    results_dict['RidgeRegression-v2-alpha10'] = results
    
    if results['test_r2'] > best_r2:
        best_r2 = results['test_r2']
        best_model_name = 'RidgeRegression-v2-alpha10'
    
    # 4. LightGBM (if available)
    if LIGHTGBM_AVAILABLE:
        results = trainer.train_lightgbm_regression(X_train_scaled, y_train, X_test_scaled, y_test)
        trainer.print_results('LightGBM-v2', results)
        results_dict['LightGBM-v2'] = results
    ...
    # Log to MLflow (optional - skip for now)
    # log_model_metrics(best_model_name, 
    #                   {'features': len(trainer.feature_names), 'version': 'v2_enriched'},
    #                   {'test_r2': best_r2, 'test_rmse': results_dict[best_model_name]['test_rmse']})
    #
    # end_run()
